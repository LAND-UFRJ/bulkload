import ouiDataRaw from './data/ouiData.json';

const ouiMap: Record<string, string> = ouiDataRaw;
...
export function enrichDeviceData(mac: string, hostnameInput: string | null, vendorClassInput: string | null): DeviceInfo {
  ...
  // --- 3. Tentativa final por OUI do MAC (se ainda não tiver Vendor) ---
  if (!vendor && mac) {
    // Remove símbolos e deixa maiúsculo
    const cleanMac = mac.replace(/[^a-fA-F0-9]/g, '').toUpperCase();
    
    // Tenta casar OUI-36 (9 chars), OUI-28 (7 chars) e OUI-24 (6 chars)
    // Nosso JSON tem chaves de tamanhos variados, então checamos
    
    // Checa 9
    if (cleanMac.length >= 9 && ouiMap[cleanMac.substring(0, 9)]) {
        vendor = ouiMap[cleanMac.substring(0, 9)];
    }
    // Checa 7
    else if (cleanMac.length >= 7 && ouiMap[cleanMac.substring(0, 7)]) {
        vendor = ouiMap[cleanMac.substring(0, 7)];
    }
    // Checa 6 (Padrão mais comum)
    else if (cleanMac.length >= 6 && ouiMap[cleanMac.substring(0, 6)]) {
        vendor = ouiMap[cleanMac.substring(0, 6)];
    }
  }
